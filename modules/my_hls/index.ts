...
fs.readdirSync(__dirname+'/tmp').forEach(v=>fs.stat(__dirname+'/tmp/'+v,(err,stat)=>{
    ...
    if(stat.isDirectory()) fs.rmdir(__dirname+'/tmp/'+v,(err)=>{});
    else fs.unlink(__dirname+'/tmp/'+v,(err)=>{});
}
))
...
class Index_Event extends Event{
    index:{m3u8:((number|string)[][]|undefined),ended:boolean}//(number|string)[][]
    constructor(data:{m3u8:((number|string)[][]|undefined),ended:boolean}){ super('index'); this.index = data }
}

class Create_Event extends Event{
    index:{m3u8:((number|string)[][]|undefined),ended:boolean}
    constructor(data:{m3u8:((number|string)[][]|undefined),ended:boolean}){ super('create'); this.index = data }
}
...
class HLS extends EventTarget{
    ...
    m3u8_url:string
    //ts_file_list:string[] = []
    m3u8_list:(number|string)[][] = []
    timeout:NodeJS.Timeout|undefined
    index_event_did:boolean = false;
    create_event_did:boolean = false;
    ...
    public create_ts(hls_time:number=10,){
        ...
        else{this.status='end'; console.log('err'); this.dispatchEvent(new Event('error'))}
        
        //폴더 만들기
        if(!fs.existsSync(this.dirpath)) fs.mkdirSync(this.dirpath)
        this.status = 'working';
        const cmd:string=`"${__dirname}\\ffmpeg.exe" -i "${this.url}" -profile:v baseline -level 3.0 -start_number 0 -hls_time ${hls_time} -hls_list_size 0 -f hls index.m3u8`
        ...
        const ffmpeg = spawn(
            ...
            ['-i',this.url ,'-profile:v','baseline', '-level','3.0', '-start_number','0', `-hls_time`,`${hls_time}`, '-hls_list_size','0', '-f','hls','index.m3u8'],
            {cwd:this.dirpath}
        );
        ...
        ffmpeg.on('close', function (code) {
            ...
            var data = _this.get_m3u8_list();
                if(!data) {_this.status='end'; _this.dispatchEvent(new Event('error')); return;}
        ...
        ffmpeg.on('err', function(err) {
            ...
            _this.dispatchEvent(new Event('error', err)); return;
        });
    ...
    public get_m3u8_list():{m3u8:((number|string)[][]|undefined),ended:boolean}{
        if (fs.existsSync(this.m3u8_url)) {
            
            const data: Buffer = fs.readFileSync(this.m3u8_url)
            //if(err) {this.status='end'; console.log(err); this.dispatchEvent(new Event('error'))}
            const dd = data.toString('utf8').trim().split('\n')
            const ended:boolean = dd[dd.length-1]=='#EXT-X-ENDLIST'

            const df = dd.filter(v => v.endsWith('.ts'))
            const ds = dd.filter(v => v.startsWith('#EXTINF:') && v.endsWith(','))
            const out: (number | string)[][] = df.map((v, i) => [df[i], Number(ds[i].replace(/^#EXTINF:(\d+.\d+),$/, '$1'))])
            this.m3u8_list = out;
    ...
    //요청에 대해 파일 반환
    public get_ts_url(i:number):(undefined|string){
    ...
    public remove():boolean{
        ...
        Promise.all(fs.readdirSync(this.dirpath).map(()=>new Promise(resolve=>(v=>fs.stat(this.dirpath+'/'+v,(err,stat)=>{
            console.log(v, stat.isDirectory());
            if(stat.isDirectory()) fs.rmdirSync(this.dirpath+'/'+v);
            else fs.unlinkSync(this.dirpath+v);
        }))))).then(()=>{
...
class HLS_manage{
    ...
    add_HLS(url:string, callback_index:(index:any)=>{}, callback_create:()=>{}){
        console.log('[HLS > HLS_manage > add_HLS], url:',url);
        if(!fs.existsSync(url)) return false;
        const hls:HLS = new HLS(url, 120)
        hls.create_ts();
        ...
        hls.addEventListener('remove',()=>{
            const index:number = this.HLS_url_list.indexOf(url)
            this.HLS_url_list.splice(index,1)
            this.HLS_list.splice(index,1)
        });
    }
    get_HTL_ts(url:string, index:number):string|undefined{
