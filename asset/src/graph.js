...
var Graph = /** @class */ (function () {
    function Graph(dom, xlabel, ylabel, type, scale_spacing) {
        ...
        this.hover_path_weight = 5; //마우스올렸을 때 변할 굵기
        this.timezoneoffset = 0;
    ...
    Graph.prototype.set_x_as_time = function (timezoneoffset) {
        this.x_is_time = true;
        if (timezoneoffset)
        ...
        if (this.type == '꺾은선') {
            y_out.forEach(function (data, i) {
                if (_this.data_show[i]) {
                    ...
                    var path_ele_1 = _this.create_path(d, _this.data_color[i], 2, ['graph_data']);
                    _this.path_ele_list[i] = path_ele_1;
                    _this.g1.appendChild(path_ele_1);
                    path_ele_1.addEventListener('mouseenter', function (e) {
                        path_ele_1.style.strokeWidth = _this.hover_path_weight + 'px';
                        var k = _this.legend_ele_list[i].querySelectorAll('text');
                        for (var i_1 = 0; i_1 < k.length; i_1++)
                            k[i_1].style.fontWeight = 'bold';
                    });
                    path_ele_1.addEventListener('mouseout', function (e) {
                        path_ele_1.style.strokeWidth = '';
                        var k = _this.legend_ele_list[i].querySelectorAll('text');
                        for (var i_2 = 0; i_2 < k.length; i_2++)
                            k[i_2].style.fontWeight = '';
                    });
                }
            });
        }
    ...
    Graph.prototype.drow_axis = function () {
        ...
        var _timezoneoffset = this.timezoneoffset;
        function myDate(x) {
            var y = [];
        ...
        if (this.x_is_time) {
            ...
            var time_id_1 = [0];
            var tmp = 0;
            for (var i_3 = 1; i_3 < time_name.length; i_3++) {
                tmp += (time_name[i_3] == time_name[i_3 - 1] ? 0 : 1);
                time_id_1.push(tmp);
            }
            //console.log(time_id)
            ...
            var label_fns = {
                ...
                get_label_text: function (pre_timeunit_list, timeunit_list) {
                    ...
                    for (var i_4 = timeunit_list.length - 1; i_4 >= 0; i_4--) {
                        if (flag || timeunit_list[i_4] != pre_timeunit_list[i_4]) {
                            flag = true;
                            out.push("" + (timeunit_list[i_4] + Number(n + i_4 == 4)) + ['초', '분', '시', '일', '월', '년'][n + i_4]);
                        }
            ...
            var _loop_1 = function () {
                ...
                if (time_id_1[timetype_1] <= 3)
                    ...
                    if (time_id_1[timetype_1] == 4)
                        v = Number(myDate(date_1.getFullYear(), date_1.getMonth() + 1));
                    else if (time_id_1[timetype_1] == 5)
                        v = Number(myDate(date_1.getFullYear() + 1, 0));
                }
    ...
    Graph.prototype.drow_legend = function () {
        ...
        var _loop_2 = function (i) {
            ...
            g.addEventListener('mouseout', function (e) {
                _this.path_ele_list[i].style.strokeWidth = ''; //'2px'
                //console.log('moveout',i, this.path_ele_list[i]);
            });
            g.addEventListener('mouseenter', function (e) {
                _this.path_ele_list[i].style.strokeWidth = _this.hover_path_weight + 'px';
                //console.log('moveenter',i, this.path_ele_list[i]);
            });
            this_2.legend_ele_list.push(g);
