        ...
        if (this.type == '꺾은선') {
            y_out.forEach((data, i) => {
                if (this.data_show[i]) {
                    ...
                    const path_ele = this.create_path(d, this.data_color[i], 2, ['graph_data']);
                    ...
                    path_ele.addEventListener('mouseenter', e => {
                        ...
                        const k = this.legend_ele_list[i].querySelectorAll('text');
                        for (let i = 0; i < k.length; i++)
                            k[i].style.fontWeight = 'bold';
                    });
                    path_ele.addEventListener('mouseout', e => {
                        path_ele.style.strokeWidth = '';
                        const k = this.legend_ele_list[i].querySelectorAll('text');
                        for (let i = 0; i < k.length; i++)
                            k[i].style.fontWeight = '';
                    });
                }
            });
        }
