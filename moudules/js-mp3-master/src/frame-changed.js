...
var Frame = {
    createNew: function (header, sideInfo, mainData, mainDataBits) {
         ...
         * @constructor
         */
        frame.decode = function (nch) {
            //console.log('[frame.decode 실행됨 ] nch',nch,frame.mainData.Is[0][0])
            frame.nch =nch
            ...
            //console.log('[decode - out length]',(out.length),'[sum]',sum)
            return out;
        };
        ...
        frame.hybridSynthesis = function (gr, ch) {
            //if(gr+ch==0)console.log('[hybridSynthesis]',frame.mainData.Is[gr][ch])
            // Loop through all 32 subbands
            //if(gr+ch==0)console.log('[antialias]',frame.store[0][0][0])
            for (var sb = 0; sb < 32; sb++) {
                ...
                //if(sb==0) if(gr+ch==0)console.log('[hybridSynthesis]',sb,frame.mainData.Is[gr][ch].slice(0,3))
                // Overlapp add with stored vector into main_data vector
                for (var i = 0; i < 18; i++) {
                    frame.mainData.Is[gr][ch][sb * 18 + i] = rawout[i] + frame.store[ch][sb][i];
                    //if(sb==0) if(gr+ch==0)console.log('[hybridSynthesis]',sb,i,frame.mainData.Is[gr][ch].slice(0,3),rawout[i] , frame.store[ch][sb][i])
                    frame.store[ch][sb][i] = rawout[i + 18];
                }
            }
            //if(gr+ch==0)console.log('[hybridSynthesis]',frame.mainData.Is[gr][ch].slice(0,3))
            //bt inData rawout 같음
        };
        ...
        frame.subbandSynthesis = function (gr, ch, out) {
            ...
            var nch = frame.nch//frame.header.numberOfChannels();
    ...
    // f.v_vec[ch][0:1024-64])
            ...
            //console.log('[subbandSynthesis]',gr,ch,out,frame.mainData.Is[0][0])
            return out;
        };
    ...
    read: function (source, position, prev) {
        ...
        console.log('[frame.js > read  - 읽고 허프만한 데이터]',result)
