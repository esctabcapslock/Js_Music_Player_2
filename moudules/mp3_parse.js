...
function mp3_parse(file,callback){
    ...
    //console.log('file',file, file_len)
   ...
   //console.log()
    ...
    while(p<file_len){
       ...
       // console.log('Padding_bit:',!!Padding_bit, Padding_bit)
        ...
        if (prev) prev.buf = Buffer(prev.buf).slice(prev.buf.length-main_data_begin, prev.buf.length)
        ...
        if(frame_cnt%300==0) console.log(frame_cnt,'[v_max]',v_max,v_min)
        if(frame_cnt%300==0) console.log(frame_cnt)
...
url = "../12.mp3"
console.log(url)
mp3_parse(fs.readFileSync(url),(v)=>{console.log(v)});
